...
class D3MCELL(nn.Module):
    def __init__(self, input_size, hidden_size, n_hidden_layer, n_axis, zhidden_size, seq_len, output_size, device):
        ...
        self.single_li = nn.Linear(hidden_size, 1)
        self.norm = nn.ModuleList([nn.LayerNorm(hidden_size) for _ in range(n_hidden_layer + 1)])
